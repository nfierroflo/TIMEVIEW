...
trial_dict={'dataset_name':"SineTransDataset_AR",'dataset_params':dataset_dict,'sequence_length':40,'prediction_length':20,'input_channels':1,'output_channels':1,'n_samples':200,'n_timesteps':60, 'B':5,'epochs':200,'n_trials':n_trials}
...
trial_dict={'dataset_name':"BetaDataset",'dataset_params':dataset_dict,'sequence_length':40,'prediction_length':20,'input_channels':1,'output_channels':1,'n_samples':900,'n_timesteps':60, 'B':5,'epochs':200,'n_trials':n_trials}
...
trial_dict={'dataset_name':"SyntheticTumorDataset",'dataset_params':dataset_dict,'sequence_length':40,'prediction_length':20,'input_channels':1,'output_channels':1,'n_samples':2000,'n_timesteps':60, 'B':9,'epochs':200,'n_trials':n_trials}
...
#FLChainDataset
dataset_dict={}
trial_dict={'dataset_name':"FLChainDataset",'dataset_params':dataset_dict,'sequence_length':10,'prediction_length':10,'input_channels':1,'output_channels':1,'n_samples':5499,'n_timesteps':20, 'B':9,'epochs':200,'n_trials':n_trials}
...
trial_dict={'dataset_name':"StressStrainDataset",'dataset_params':dataset_dict,'sequence_length':140,'prediction_length':70,'input_channels':1,'output_channels':1,'n_samples':47,'n_timesteps':210,
                "fixed_len":210, 'B':9,'epochs':200,'n_trials':n_trials,'Ti':0.0,'T':1.0}

execute_trials(**trial_dict)

#Sines sum
print("QuadraticSinesum")
sequence_length = 40
prediction_length = 20
input_channels = 2
output_channels = 1
...
#give the two X1 and X2 to the model
X_torch=torch.cat((X1_torch,X2_torch),dim=2)
print(X_torch.shape)
...
new_row = pd.DataFrame({'dataset': ['QuadraticSinesum'], 'model': ['TSMixer'], 'mean_loss': [mean_loss], 'std_loss': [std_loss]})
# Concatenate the new row
results = pd.concat([results, new_row], ignore_index=True)
B=5
...
new_row = pd.DataFrame({'dataset': ['QuadraticSinesum'], 'model': ['TIMEVIEW4FE(TSMixer)'], 'mean_loss': [mean_loss], 'std_loss': [std_loss]})
# Concatenate the new row
results = pd.concat([results, new_row], ignore_index=True)
os.chdir('../TIMEVIEW4FE')
#save the results to a csv file
results.to_csv('results/AutoRegressive_results.csv',index=False)
