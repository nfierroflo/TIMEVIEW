...
def spline_coeff_loss_seasonality(criterion, output, Y_batch, Phis,ts):
...
def train_latent_tsmixer_seasonality(
    model,
    X_torch,
    Y_torch,
    Phis,
    ...
    for epoch in range(epochs):
        ...
        for X_batch, Y_batch in train_loader:
            ...
            loss = spline_coeff_loss_seasonality(criterion, output, Y_batch, Phis,ts)
            # Backward pass
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
